...
function searchBook() {
    ...
    var url = "get_book.php?title=" + encodeURI(title);
...
function add_book_post() {
    ...
    fetch('add_book_post.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.text())
    .then(result => {
        document.getElementById('add_response').innerHTML = result;
    })
...
function add_book_get() {
    ...
    fetch('add_book_get.php?' + queryString, {
        method: 'GET'
    })
    .then(response => response.text())
    .then(result => {
